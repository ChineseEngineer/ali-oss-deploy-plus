...
class aliOssDeployPlus {
  ...
  constructor(options) {
    ...
    _defineProperty(this, "resourceFiles", []);

    _defineProperty(this, "htmlFiles", []);

    _defineProperty(this, "ossFiles", []);

    _defineProperty(this, "currentProjectPath", '');

    _defineProperty(this, "promiseLimit", null);

    const {
      ossOpts,
      proOpts
    } = options;
    this.proOpts = { ...this.proOpts,
      ...proOpts
    };
  ...
  setItem(name, val, prop) {
    ...
    this.resourceFiles.splice(index, 1, { ...i,
      [prop]: val
    });
  }
  ...
  uploadFiles() {
    ...
    files.forEach((i, index) => {
      uploadQueue.push(this.promiseLimit(() => this.client.put(`${this.proOpts.projectPath}/${i.name}`, `${this.proOpts.currentProjectPath}/${i.name}`).then(res => {
        try {
    ...
    p.then(res => {
      ...
      if (failedUploadQueue.length) {
        divider('文件上传失败列表：');
        divider([...failedUploadQueue].map(item => `失败：${item.name}`).join("\n"));
        ++this.retryTimes;
        ...
        divider('上传html文件'); // 延迟1s更新 html文件
        ...
        console.timeEnd('耗时'); //  删除增量部署遗留垃圾文件
    ...
    this.getOssFilesByPrefix().then(res => {
      ...
      files.forEach(i => {
        ...
        if (!fs.existsSync(`${this.proOpts.currentProjectPath}/${i.name.replace(this.proOpts.projectPath, '')}`) && lastModified.getTime() < cutoffTime) {
          ...
          deleteQueue.push(this.promiseLimit(() => this.client.delete(i.name).then(res => {
            log(`[删除成功]: ${i.name}`);
            return `${i.name}`;
          }, err => {
            log(`[删除失败]：${i.name}`);
