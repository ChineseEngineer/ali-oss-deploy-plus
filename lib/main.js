...
class aliOssDeployPlus {
  ...
  constructor(options) {
    ...
    _defineProperty(this, "targetDir", null);
  ...
  uploadFiles() {
    ...
    files.forEach((i, index) => {
      const projectPath = this.proOpts.PROJECT ? `${this.proOpts.ENTRY}/${this.proOpts.PROJECT}/${i.name}` : `${this.proOpts.ENTRY}/${i.name}`;
      uploadQueue.push(this.promiseLimit(() => this.client.put(projectPath, `${this.proOpts.targetDir}/${i.name}`).then(res => {
        try {
    ...
    p.then(res => {
      ...
      if (failedUploadQueue.length) {
        ...
        divider('上传html文件'); // 延迟2s更新 html文件
  ...
  isExists(path) {
    try {
      ...
      log(e); // console.log(error(e.message))
    }
